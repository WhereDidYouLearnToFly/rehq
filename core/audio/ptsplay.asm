;=============================================================================
; ptsplay - VENDORED third-party module tracker player, do not restyle
;=============================================================================
; Universal PT2/PT3/TurboSound player by S.V.Bulba, taken verbatim from
; ZiFi (_pt/PTSPlay.asm, https://github.com/psbhlw/ZiFi). Only this header
; was added. Keep it that way: diffing against upstream is the only practical
; way to audit 1700 lines of someone else's self-modifying player, so the
; house style in the rest of core/ deliberately stops at this file.
;
; Placement:  no ORG, but it must live in RAM - INIT and PLAY both rewrite
; their own code, so a copy in ROM or in a write-protected page will not run.
; Roughly $BC7 (3015) bytes, code plus variables.
; Depends on: nothing.
; Namespace:  MODULE PTS.
;
; Do not call it directly from game code - core/audio/music.asm wraps the
; three entries below with the housekeeping they need (IY, muting, choosing
; between formats). What that wrapper is built on:
;
;   PTS.INIT   hl = module, de = second module for TurboSound, a = SETUP.*
;              flags. Must be called before PLAY or the player will crash on
;              uninitialised variables.
;   PTS.PLAY   advance one frame and write the AY registers. Sets IY, which
;              the 48K ROM interrupt handler needs pointed at $5C3A - see
;              music.frame.
;   PTS.MUTE   silence the chip without forgetting the position.
;
; The module data itself is format-checked nowhere here; upstream keeps the
; PT2/PT3 header validators in separate files (_pt/pt2.asm, _pt/pt3.asm),
